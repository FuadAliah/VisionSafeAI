    ...
    <section className="py-24 px-6 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className={isAr ? 'order-2' : ''}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {data.problemStatement.stats.map((stat, idx) => (
                <div key={idx} className="bg-white border-2 border-gray-100 p-6 rounded-2xl hover:border-purple-300 transition-all">
                  ...
                  {stat.sourceUrl && (
                    <a href={stat.sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-purple-500 hover:underline font-medium">
          ...
          <div className={`bg-gradient-to-br from-purple-50 to-white p-8 rounded-3xl border-2 border-purple-100 h-[500px] flex flex-col ${isAr ? 'order-1' : ''}`}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} layout="vertical">
                ...
                />
                <Bar dataKey="numericValue" radius={[0, 12, 12, 0]} barSize={50}>
                  {chartData.map((_, index) => (
                    <Cell key={`cell-${index}`} fill={index % 2 === 0 ? '#a855f7' : '#9333ea'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
